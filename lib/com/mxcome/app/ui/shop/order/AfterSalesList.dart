...
class AfterSalesList extends StatefulWidget {
  ...
  AfterSalesList(this.status, {super.key});

  @override
  State<AfterSalesList> createState() => AfterSalesListState();
}

class AfterSalesListState extends BaseKeepAliveState<AfterSalesList> {
  ...
  Future<void> loadContentDatas() async {
    ...
    BaseRsp rsp = await HttpUtils.post(IURLConstant.MALL_AFTER_SALES_LIST,  {
      "afterSalesStatus": widget.status, "pageNum": "$page", "pageSize": "10"
    });
  ...
  Widget buildStatus(dynamic order) {
    int afterSalesStatus = BaseModel.getInt(order, "status");
