...
import '../widget/BuyProgressBar.dart';
...
class ActivityDetailPage extends StatefulWidget {
  ...
  ActivityDetailPage(this.activityMember);

  @override
  State<ActivityDetailPage> createState() => ActivityDetailPageState();

}

class ActivityDetailPageState extends BaseKeepAliveState<ActivityDetailPage> {
  ...
  Future<void> loadContentDatas() async {
    ...
    String endTime = BaseModel.getString(_activityMember, "endTime");
    _endTime = DateTime.parse(endTime);
    _countedTime = _endTime.add(Duration(minutes: _countedTimeout));
    String activityId = BaseModel.getString(_activityMember, "activityId");
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    SizedBox(width: 10.w),
                    Offstage(offstage: isActivityEnd(), child: Image.asset("assets/icons/task_icon1.png", width: 11.w, height: 11.w)),
                    SizedBox(
                      width: 4.w,
                    ),
                    CountDownView(startTime: serviceTime, endTime: "${BaseModel.getString(_activityDetail, "endTime")}",
                        textColor: IConstant.main_color,
                        textAlign: TextAlign.right,
                        prefixColor: IConstant.sub_text_color,
                        prefix: LanguageConfig.get(LanguageConfigKeys.Shop_activity_count_down),
  ...
  bool isActivityEnd() {
    return statusText == LanguageConfig.get(LanguageConfigKeys.Shop_activity_wait_statistics_completed) || statusText == LanguageConfig.get(LanguageConfigKeys.Shop_activity_congratulations_get_prize);
  }

  Widget buildClock(dynamic item) {
    if (_filterPocketMemberList.isNotEmpty) {
      String endTime = BaseModel.getString(_filterPocketMemberList[0], "endTime");
      return CountDownView(startTime: serviceTime, endTime: endTime, fontSize: 12.sp,
          textColor: IConstant.main_color,
  ...
  Future<void> setBtnStatus() async {
    int total = 0;
    for (dynamic item in _pocketMemberList) {
      int activityLevel = BaseModel.getInt(item, "activityLevel");
    ...
    if (total >= 1) { //完成
      if (_currentLevel == _levelNum) { //领奖台
        ...
        if (!Util.isTimeout2(startTime: serviceTime, endTime: FormatUtil.formatLineYMDHMS(_endTime))) {
          statusText = LanguageConfig.get(LanguageConfigKeys.Shop_activity_continue_recommend); //活动结束前，可以继续推荐
        } else if(!Util.isTimeout2(startTime: serviceTime, endTime: FormatUtil.formatLineYMDHMS(_countedTime))){
          statusText = LanguageConfig.get(LanguageConfigKeys.Shop_activity_wait_statistics_completed); //恭喜，请耐心等待统计完成
        }
        else {
          statusText = LanguageConfig.get(LanguageConfigKeys.Shop_activity_congratulations_get_prize); //恭喜完成，点击领奖
        }
      ...
      child: SizedBox(
        ...
        child: Stack(
          children: [
                ...
                children: [
                  Expanded(child: Row(
                    ...
                    children: [
                      ...
                      SizedBox(width: 8.w),
                      PriceText(BaseModel.getDynamic(_activityMember, "withdrawalBalance")??0, fontSize: 14.sp),
                    ],
                  )),
                  SizedBox(width: 16.w),
                  buildNextBtn()
            ...
            statusText == LanguageConfig.get(LanguageConfigKeys.Shop_activity_one_step_success) || statusText == LanguageConfig.get(LanguageConfigKeys.Shop_activity_congratulations_get_prize) ? Container() :
  ...
  Widget buildNextBtn() {
    if (_levelStatus == 0) {
      ...
      if (Util.isTimeout2(startTime: serviceTime, endTime: FormatUtil.formatLineYMDHMS(_endTime)) && !Util.isTimeout2(startTime: serviceTime, endTime: FormatUtil.formatLineYMDHMS(_countedTime))) {
        return IconTextButton(
            icon: Image.asset(btnIcon, color: IConstant.white_color, width: 22.w, height: 22.w),
            bgColor: IConstant.red_bg_color5,
            textColor: IConstant.red_bg_color4,
            fontSize: 13.w,
            text: btnText, onTap: () {
              nextPage(PodiumPage(_activityDetail, _activityMember), false);
        });
      ...
      children: [
        ...
        Expanded(child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                BuyProgressBar(progress: getBuyQuantity(pocketMember), color: color),
                expandeSpace,
                RichText(text: TextSpan(children: [
                  ...
                  TextSpan(text: "${ getBuyQuantity(pocketMember) }", style: TextStyle(fontSize: 12.sp, fontWeight: FontWeight.bold, color: IConstant.main_color)),
                  WidgetSpan(child: Container(
                      margin: EdgeInsets.fromLTRB(10.w, 0.w, 10.w, 0.w),
                      width: 1.w, height: 12.w, color: IConstant.line_color)),
                  TextSpan(text: "${ getStock(pocketMember) }", style: TextStyle(fontSize: 12.sp, color: IConstant.text_color)),
                ])),
                SizedBox(width: 14.w),
              ],
            )
  ...
  int getBuyQuantity(dynamic item) {
    return BaseModel.getInt(item, "buyQuantity");
  }

  int getStock(dynamic item) {
    return BaseModel.getInt(item, "stock");
  }
  ...
  void showNextDialog() {
    showPop(0.4 * Adapt.getWindowHeight(), CommResultPage(
        title: LanguageConfig.get(LanguageConfigKeys.Shop_activity_successfully_crossed),
        btnText: LanguageConfig.get(LanguageConfigKeys.Shop_activity_enter_next_level),
        message: Padding(padding: EdgeInsets.fromLTRB(20.w, 10.w, 20.w, 10.w),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.center,
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Icon(Icons.check_circle_outline, size: 45.w, color: IConstant.main_color),
              SizedBox(height: 30.w,),
              Text(LanguageConfig.get(LanguageConfigKeys.Shop_activity_successfully_crossed_tip),
  ...
  Future<void> showSuccessDialog() async {
    String activityId = BaseModel.getString(_activityMember, "activityId");
    String successData = await AppUtils.getActivitySuccess(activityId);
    if (TextUtils.isEmpty(successData)) {
      showPop(0.6 * Adapt.getWindowHeight(), CommResultPage(
          title: LanguageConfig.get(LanguageConfigKeys.Shop_activity_all_level_success),
          message: Column(
            crossAxisAlignment: CrossAxisAlignment.center,
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              ...
              Padding(padding: EdgeInsets.fromLTRB(20.w, 30.w, 20.w, 10.w),
                child: Text(_status == 1 ? LanguageConfig.get(LanguageConfigKeys.Shop_activity_all_level_success_tip2): LanguageConfig.get(LanguageConfigKeys.Shop_activity_all_level_success_tip),
                    textAlign: TextAlign.center,
                    style: TextStyle(fontSize: 14.sp, color: IConstant.title_color)),
              )
            ],
          ...
          callBack: (BuildContext ctx) async {
            finishContext(ctx);
            await AppUtils.setActivitySuccess(activityId);
          }), enableDrag: false
      );
    }
  }
  ...
  void showFailedDialog() {
    if(!_failedDialog && _status == -1) {
      _failedDialog = true;
      ViewUtils.showCustomDialog(context, LanguageConfig.get(LanguageConfigKeys.Shop_activity_obsolete),
          Column(
            crossAxisAlignment: CrossAxisAlignment.center,
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Image.asset("assets/icons/activity_failed.png", width: double.infinity, height: 180.w),
              SizedBox(height: 20.w),
              Text(LanguageConfig.get(LanguageConfigKeys.Shop_activity_obsolete_tip), textAlign: TextAlign.center,
                  style: TextStyle(fontSize: 13.sp, color: IConstant.text_color)),
            ],
          ),
          LanguageConfig.get(LanguageConfigKeys.Shop_activity_homepage), (ctx, event) {
