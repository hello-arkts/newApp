...
class AssetTransferPage extends StatefulWidget {
  ...
  AssetTransferPage(this.walletList);

  @override
  State<StatefulWidget> createState() => AssetTransferPageState();

}

class AssetTransferPageState extends BaseKeepAliveState<AssetTransferPage> {
    ...
    return KeyboardDismissOnTap(
      child: Scaffold(
        appBar: AppBar(
          elevation: 0.w,
          centerTitle: true,
          title: Text(LanguageConfig.get(LanguageConfigKeys.Shop_web3_transfer),
        ...
        body: Column(
          ...
          children: [
            Padding(padding: EdgeInsets.only(left: 16.w, right: 16.w, top: 16.w),
              child: Text(LanguageConfig.get(LanguageConfigKeys.Shop_web3_account_email), style: TextStyle(fontSize: 15.sp, color: IConstant.text_color)),),
            Container(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                    ...
                    hintText: LanguageConfig.get(LanguageConfigKeys.Shop_web3_enter_email),
                ...
                controller: ViewUtils.buildTextEditingController(_email, listener: (str) {
                  _email = str;
                  checkInput();
                }),
              ),
  ...
  void checkInput() {
    if (TextUtils.isNotEmpty(_email) && _selectList.isNotEmpty) {
  ...
  void transfer() {
    if (!NumUtils.isEmail(_email)) {
      ViewUtils.showToastLong(LanguageConfig.get(LanguageConfigKeys.shop_web3_format_incorrect));
      return;
    }
    showPop(0.5 * Adapt.getWindowHeight(), ActionSliderPage(_selectCoinInfo, _selectList, email: _email));
  }
